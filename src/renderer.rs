...
fn setup_renderer(
    ...
    raw_handle_wrappers: Query<&RawHandleWrapper>,
) -> Result<(), BevyError> {
    let window = windows.single()?;
    let UVec2 { x, y } = window.physical_size();
    let raw_handle_wrapper = raw_handle_wrappers.single()?;
    let window_handle = raw_handle_wrapper.get_window_handle();
    let display_handle = raw_handle_wrapper.get_display_handle();
    let renderer = Renderer::new(window_handle, display_handle, x, y)?;
